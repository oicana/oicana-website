...
description: Integrate Oicana into a Node.js web service using NestJS.
---
...
1. Add the [`@oicana/node` npm package](https://www.npmjs.com/package/@oicana/node) as a dependency with `npm install @oicana/node`.
2. Generate a new controller and service for templates:
...
3. Copy `example-0.1.0.zip` into the generated `src/templates` directory, then tell the Nest CLI to carry it into the build output:

   ```json title="Part of nest-cli.json"
   "compilerOptions": {
     "deleteOutDir": true,
     "assets": ["**/*.zip"]
   }
...
4. Update the templates service to load the template at startup:
   ...
   ```typescript title="src/templates/templates.service.ts"
   ...
   export class TemplatesService implements OnModuleInit {
     ...
     async onModuleInit() {
       const templatePath = join(import.meta.dirname, 'example-0.1.0.zip');
       const buffer = await fs.readFile(templatePath);
       // The warm-up compilation that registration runs defaults to
       // Development mode and thus uses the fallback value for the input.
...
5. Update the templates controller to add a compile endpoint:
   ...
   ```typescript title="src/templates/templates.controller.ts"
...
```typescript title="Part of src/templates/templates.service.ts"
compile(): Uint8Array {
  const jsonInputs = new Map<string, string>();
  const blobInputs = new Map<string, BlobWithMetadata>();
...
A missing required input or an input that fails schema validation makes the compilation fail. The tutorial's `example` template declares no [JSON schema](/docs/templates/inputs/#json-schemas), so only the first case can happen here. `export()` throws exceptions that we can catch:

```typescript title="Part of src/templates/templates.controller.ts"
import {
  Controller,
...
```typescript title="src/templates/templates.service.ts"
import { Injectable, OnModuleInit } from '@nestjs/common';
import { Template, type BlobWithMetadata } from '@oicana/node';
import { promises as fs } from 'fs';
import { join } from 'path';
