...
For better performance in production environments with heavy load, consider moving compilation to worker threads. This allows you to offload CPU-intensive compilation work from the main event loop. Libraries like [piscina](https://github.com/piscinajs/piscina) can help with that.

## Passing inputs from Node.js

Our `compile` method is currently calling `template.compile()` with empty input maps and development mode. Now we'll provide an explicit input value and switch to production mode:
