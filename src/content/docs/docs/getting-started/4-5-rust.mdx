---
title: Rust / Axum
description: Integrate Oicana into a Rust web service using axum.
---
...
2. Replace the contents of `src/main.rs` with a basic Axum server that loads and compiles the template:
   ...
   const TEMPLATE: &[u8] = include_bytes!("../templates/example-0.1.0.zip");

   #[tokio::main]
   async fn main() {
       let template = Template::init(Cursor::new(TEMPLATE))
...
<Aside title="No automatic warm-up compilation">
`Template::init` only reads the template, it does not compile it. The first `compile()` call therefore pays the full compilation cost, and a required input without a `development` or `default` value fails there rather than at startup. Call `compile()` once after `init` if you want the same warm-up the other integrations run at template registration.
</Aside>
...
```rust title="Part of src/main.rs"
async fn compile(State(template): State<Arc<Mutex<Template<PackedTemplate>>>>) -> impl IntoResponse {
    let mut template = template.lock().unwrap();
...
A missing required input or an input that fails schema validation makes the compilation fail. The tutorial's `example` template declares no [JSON schema](/docs/templates/inputs/#json-schemas), so only the first case can happen here.
...
let result = match template.compile(inputs) {
    Ok(result) => result,
    Err(error @ CompileError::ValidationFailed(_)) => {
        eprintln!("Rejected inputs: {error}");
        return StatusCode::BAD_REQUEST.into_response();
    }
    Err(error @ CompileError::CompilationFailed(_)) => {
        eprintln!("Failed to compile template: {error}");
        return StatusCode::INTERNAL_SERVER_ERROR.into_response();
    }
};
...
`ValidationFailed` is the caller's mistake and deserves a 4xx; `CompilationFailed` is a problem with the template or a missing input. Compilation errors are multi-line and quote the offending lines of your template.

A compilation can also produce warnings without failing, for example when a template asks for a font that is not available and Typst falls back. Those do not reach `stderr` on their own: check `result.warnings` and log it when it is `Some`. See [compilation warnings](/docs/integrations/#compilation-warnings) for why that matters.
...
async fn compile(State(template): State<Arc<Mutex<Template<PackedTemplate>>>>) -> impl IntoResponse {
    ...
    if let Some(warnings) = &result.warnings {
        eprintln!("Template compiled with warnings: {warnings}");
    }
