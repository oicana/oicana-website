---
title: "C# / ASP.NET"
description: Integrate Oicana into a C# web service using ASP.NET Core.
---

import { Aside } from '@astrojs/starlight/components';
import { Image } from 'astro:assets';
import swaggerResponse from '../../../../assets/swagger_response.png';
...
1. Create a new directory in the .NET project called `templates` and copy `example-0.1.0.zip` into that directory. Then tell MSBuild to ship it, otherwise `dotnet publish` leaves it behind and the published app fails at startup:

   ```xml title="Part of your .csproj"
   <ItemGroup>
...
3. Add the Oicana usings next to the `using Scalar.AspNetCore;` you already have at the top of `Program.cs`:

   ```cs title="Top of Program.cs"
   ...
   Then read the template file and prepare it for compilation, above `var builder = WebApplication.CreateBuilder(args);`:

   ```cs title="Part of Program.cs"
   var templateFile = await File.ReadAllBytesAsync(
       Path.Combine(AppContext.BaseDirectory, "templates", "example-0.1.0.zip"));
...
<Image src={swaggerResponse} alt='The 200 response from calling the compile endpoint in the Scalar API explorer. The returned PDF is rendered and can be downloaded.' />
...
A missing required input or an input that fails schema validation makes the compilation fail. The tutorial's `example` template input declares no [JSON schema](/docs/templates/inputs/#json-schemas), so only the first case can happen here. `Export` throws an `OicanaException` that we can catch:

```cs title="Part of Program.cs"
app.MapPost("/compile", (ILogger<Program> logger) =>
{
    var jsonInputs = new Dictionary<string, JsonNode>
    {
        ["info"] = JsonNode.Parse("{ \"name\": \"Baby Yoda\" }")!
