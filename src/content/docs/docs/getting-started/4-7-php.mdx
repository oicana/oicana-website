...
2. The following commands add the `oicana/oicana` Composer package as a dependency. The package is hosted on a custom Composer repository, so we need to register it, configure the minimum stability, allow the Oicana installer plugin, and finally install the package:
   ...
   composer require oicana/oicana:^0.1.0-alpha.5
   ```
   ...
   You can re-display this command at any time by executing `vendor/bin/oicana-env`
   </Aside>
