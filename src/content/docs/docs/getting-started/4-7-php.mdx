...
2. The following commands add the `oicana/oicana` Composer package as a dependency. The package is hosted on a custom Composer repository, so we need to register it, allow the Oicana installer plugin, and finally install the package:
   ...
   composer require oicana/oicana:^0.8.0
   ```
...
3. Update `index.php` to load the template at startup and add a compile endpoint:
   ...
   $templateBytes = @file_get_contents(__DIR__ . '/templates/example-0.1.0.zip');
   if ($templateBytes === false) {
       http_response_code(500);
       exit('Failed to load template file. Make sure templates/example-0.1.0.zip exists.');
   }
...
The PDF generation should not take longer than a couple of milliseconds. You can measure the compilation time with PHP's built-in `microtime(true)` around the `export()` call.
...
A missing required input or an input that fails schema validation makes the compilation fail. The tutorial's `example` template declares no [JSON schema](/docs/templates/inputs/#json-schemas), so only the first case can happen here. `export()` throws a plain `\Exception` that we can catch:
