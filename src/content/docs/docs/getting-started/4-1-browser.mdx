...
<Aside>
The WASM file included in `@oicana/browser-wasm` is about 36 MB uncompressed (~15 MB gzipped, ~11 MB brotli). It's fetched once and browser-cached. For more about handling the WASM file read the [Deploying the Browser WASM](/docs/guides/browser-deployment/) guide. It covers pre-compression, CDN caveats, Web Worker offload, and a few other things that are easy to get wrong.
</Aside>
