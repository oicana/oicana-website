...
<Aside>
This chapter assumes that you have a working Node.js 20.19+ (or 22.12+) setup with npm. If that is not the case, please follow [the official Node.js installation guide](https://nodejs.org/en/download/) to install Node.js on your machine.
...
The scaffold wraps the app in `<StrictMode>`, so React runs the effect twice in development. That is two fetches and two `Template` instances, of which the first is dropped without being released. Cleaning that up takes some care. For example, you could do the following:

```tsx
useEffect(() => {
  let created: Template | undefined;
  let cancelled = false;
  ...
  async function setup() {
    ...
    const response = await fetch('/example-0.1.0.zip');
    const templateBytes = new Uint8Array(await response.arrayBuffer());
    created = new Template(templateBytes);
    if (cancelled) {
      created.dispose();
      return;
    }
