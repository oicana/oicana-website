...
<Aside>
The WASM file included in `@oicana/browser-wasm` is about 36 MB uncompressed (~15 MB gzipped, ~11 MB brotli). It's fetched once and browser-cached, but make sure your server or CDN serves it with compression enabled and consider showing a loading state on first visit.
</Aside>
...
To keep the UI responsive, it's recommended to move template compilation to a [Web Worker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API). This prevents the main thread from being blocked during compilation. Take a look at the [open source React example project on GitHub](https://github.com/oicana/oicana-example-typescript-react/) for a Web Worker setup.
