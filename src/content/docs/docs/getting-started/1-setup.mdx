...
description: Install the Oicana CLI and set up an editor for template development with Typst.
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="tip" title="Do you need a license to follow this guide?">
For commercial use, the first 30 days are free for evaluation. Follow this guide, build a proof of concept, and test Oicana internally without a subscription. See [Section 4.2 of our Terms of Service](/terms-of-service/#42-evaluation-use) for the full terms.
...
Install a [prebuild binary from GitHub](https://github.com/oicana/oicana/releases/tag/oicana_cli-v0.3.0) or let a script do that for you.

Bash script:

```bash title="Shell script to install Oicana CLI"
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/oicana/oicana/releases/download/oicana_cli-v0.3.0/oicana_cli-installer.sh | sh
```

Powershell script:

```powershell title="Powershell script to install Oicana CLI"
-ExecutionPolicy Bypass -c "irm https://github.com/oicana/oicana/releases/download/oicana_cli-v0.3.0/oicana_cli-installer.ps1 | iex"
```
