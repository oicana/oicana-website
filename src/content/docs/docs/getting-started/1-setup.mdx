...
Install a [prebuild binary from GitHub](https://github.com/oicana/oicana/releases/tag/oicana_cli-v0.5.0) or let a script do that for you.

Bash script:

```bash title="Shell script to install Oicana CLI"
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/oicana/oicana/releases/download/oicana_cli-v0.5.0/oicana_cli-installer.sh | sh
```

Powershell script:

```powershell title="Powershell script to install Oicana CLI"
-ExecutionPolicy Bypass -c "irm https://github.com/oicana/oicana/releases/download/oicana_cli-v0.5.0/oicana_cli-installer.ps1 | iex"
```
