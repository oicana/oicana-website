...
description: Learn how to define JSON and blob inputs for your Oicana template.
---
...
```typst title="Beginning of main.typ"
...
<Aside title="Why the `read-project-file` wrapper?">
...
While we develop the template, there is no Oicana integration setting a value for it. Inputs are required unless configured otherwise, so the template does not compile yet: `setup` stops with `No value for the required input 'info' was supplied`, in the editor preview as well as in the CLI. We fix that in the next section by defining a `default` or `development` value for the input.
...
<Aside title="When to use each mode">
...
- In most integrations, template registration (typically at application startup) uses development mode when warming up the Typst cache for the template to speed up later compilations.
