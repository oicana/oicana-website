...
Oicana for Java loads a native library, and JDK 24 and newer restrict that. Out of the box you get a warning on every start:
...
- **On the module path**: Oicana ships as the named module `com.oicana`, so you can narrow the grant to `--enable-native-access=com.oicana` instead of opening up the whole classpath.
