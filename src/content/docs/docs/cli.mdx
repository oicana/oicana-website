...
[CLI builds are published on GitHub](https://github.com/oicana/oicana/releases/tag/oicana_cli-v0.1.0-alpha.16). You can pick and install the correct binary yourself or let a script do it for you.

Bash script:

```bash title="Script to install Oicana CLI"
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/oicana/oicana/releases/download/oicana_cli-v0.1.0-alpha.16/oicana_cli-installer.sh | sh
```

Powershell script:

```powershell title="Powershell script to install Oicana CLI"
-ExecutionPolicy Bypass -c "irm https://github.com/oicana/oicana/releases/download/oicana_cli-v0.1.0-alpha.16/oicana_cli-installer.ps1 | iex"
```
