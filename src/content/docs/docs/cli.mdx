...
[CLI builds are published on GitHub](https://github.com/oicana/oicana/releases/tag/oicana_cli-v0.1.0-alpha.9). You can pick and install the correct binary yourself or let a script do it for you.

Bash script:

```bash title="Script to install Oicana CLI"
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/oicana/oicana/releases/download/oicana_cli-v0.1.0-alpha.9/oicana_cli-installer.sh | sh
```

Powershell script:

```powershell title="Powershell script to install Oicana CLI"
-ExecutionPolicy Bypass -c "irm https://github.com/oicana/oicana/releases/download/oicana_cli-v0.1.0-alpha.9/oicana_cli-installer.ps1 | iex"
```
...
By default, the test directory (configured via `tool.oicana.tests`, defaults to `tests/`) and the `output/` directory are excluded from the packed template. You can customize which files are excluded using the [`exclude`](https://github.com/typst/packages/blob/main/docs/manifest.md) field in your `typst.toml` manifest:
...
The patterns use [gitignore semantics](https://git-scm.com/docs/gitignore). Patterns are applied recursively by default; prepend `/` to anchor a pattern to the template root. Keep in mind that all files required during compilation need to be packed!
