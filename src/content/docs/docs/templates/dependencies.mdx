...
If a package can't be resolved, like a local package that isn't installed in the registry, or an `@preview` package whose download fails, `oicana pack` aborts with `Failed to resolve package <spec>` and writes no archive. Otherwise, a partial archive could fail at runtime in production. Make sure every imported package is installed (or reachable from Typst Universe) before packing.
