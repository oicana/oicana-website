...
<Aside title="Watch mode">
Pass `--watch` (or `-w`) to keep the test runner alive: it executes the suite once, then re-runs the affected tests whenever a source file changes. Useful while iterating on a template — `oicana test --watch` covers both the "did I break a snapshot?" and "did the manifest still parse?" checks on every save.
</Aside>
