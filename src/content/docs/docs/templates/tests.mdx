...
snapshot = "my_snapshot.png" # Optional, default "<test-name>.png" or collection level `snapshot` if set - relative path to a png file that will be compared to the test output or `false` to disable snapshot comparison
