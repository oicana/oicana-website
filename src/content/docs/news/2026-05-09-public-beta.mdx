...
Generating PDFs from application code is usually a pick-your-poison choice. HTML-to-PDF tools have fragile pagination and weak typography, PDF libraries make verbose layouts part of your application code, and SaaS PDF builders come with proprietary binary formats, vendor lock-in and often high license cost.

Oicana takes a different route. Templates use the open source Typesetter [Typst](https://typst.app/). Your application passes typed inputs into the template, and Oicana compiles a PDF in milliseconds.
